...
@objc class AppDelegate: FlutterAppDelegate {
    ...
    override func application(
        ...
        channel.setMethodCallHandler { [weak self] (call: FlutterMethodCall, result: @escaping FlutterResult) in
            ...
            case "getForgotPunchOutIOS":
                self?.navigateToVideoControllerForgotPunchOut(completion: { (resultData) in
                    result(resultData)
                })
    ...
    func navigateToVideoControllerPunchIn(completion: @escaping (_ result: Any) -> Void) {
        let storyboard = UIStoryboard(name: "VideoCapture", bundle: nil) // Update with your storyboard name
